...
func LinkForm() templ.Component {
	return templ.ComponentFunc(func(ctx context.Context, templ_7745c5c3_W io.Writer) (templ_7745c5c3_Err error) {
		...
		_, templ_7745c5c3_Err = templ_7745c5c3_Buffer.WriteString("<script>\n\n\tconst urlInput = document.getElementById('url');\n\n\tconst warning = document.getElementById('url-valid');\n\turlInput.addEventListener('input', function (event) {\n\t\tif (isValidURL(urlInput.value) === false){\n\t\t\turlInput.setCustomValidity('Please input a valid url');\n\t\t} else {\n\t\t\turlInput.setCustomValidity('');\n\t\t}\n\t});\n\turlInput.addEventListener(\"keyup\", function (event) {\n\t\tif (isValidURL(urlInput.value) === false) {\n\t\t\turlInput.setCustomValidity('Please input a valid url');\n\t\t} else {\n\t\t\turlInput.setCustomValidity('');\n\t\t}});\n    function isValidURL(url) {\n        // Regular expression to match URLs\n        var urlPattern = /^(https?|ftp):\\/\\/[^\\s/$.?#].[^\\s]*$/i;\n        return urlPattern.test(url);\n    }\n</script><form hx-post=\"/create\" class=\"flex flex-col space-y-4 w-1/2\"><input type=\"text\" name=\"title\" placeholder=\"Title\" class=\"rounded-sm\"> <input type=\"text\" name=\"url\" id=\"url\" placeholder=\"URL\" class=\"rounded-sm\"> <span id=\"url-valid\" class=\"text-red-500\"></span><div class=\"flex justify-between\">")
