...
func (aq *AccountQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Account, error) {
	...
	if query := aq.withUsers; query != nil {
		if err := aq.loadUsers(ctx, query, nodes, nil,
			func(n *Account, e *Users) { n.Edges.Users = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (aq *AccountQuery) loadUsers(ctx context.Context, query *UsersQuery, nodes []*Account, init func(*Account), assign func(*Account, *Users)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Account)
	for i := range nodes {
		fk := nodes[i].ExternalID
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "external_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (aq *AccountQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := aq.ctx.Fields; len(fields) > 0 {
		...
		if aq.withUsers != nil {
			_spec.Node.AddColumnOnce(account.FieldExternalID)
		}
	}
